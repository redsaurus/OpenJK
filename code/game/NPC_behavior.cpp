...
void NPC_BSFollowLeader_UpdateEnemy(void)
{
	if ( !NPC->enemy  )
	{//no enemy, find one
		NPC_CheckEnemy( (NPCInfo->confusionTime<level.time)&&(NPCInfo->insanityTime<level.time), qfalse );//don't find new enemy if this is tempbehav
		if ( NPC->enemy )
		{//just found one
	...
	else
	{
		...
		{
			NPC_CheckEnemy( (((NPCInfo->confusionTime<level.time)&&(NPCInfo->insanityTime<level.time))||NPCInfo->tempBehavior!=BS_FOLLOW_LEADER), qfalse );//don't find new enemy if this is tempbehav
		}
	}
}
